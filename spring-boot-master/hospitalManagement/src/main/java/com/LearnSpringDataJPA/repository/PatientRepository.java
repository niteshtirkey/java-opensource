...
public interface PatientRepository extends JpaRepository<Patient,Long> {
    ...
    List<Patient> findByNameContainingOrderByNameDesc(String query);

    @Query("SELECT p FROM Patient p where p.bloodGroup = :bloodGroup")
    List<Patient> findBloodGroup(@Param("bloodGroup") BloodGroupType bloodGroup);

    List<Patient> findByBloodGroup(BloodGroupType bloodGroupType);

    @Query("SELECT p FROM Patient  p where p.birthDate > :birthDate")
    List<Patient> findByBornAfterDate(@Param("birthDate") LocalDate birthDate);

    @Query("SELECT new com.LearnSpringDataJPA.dto.BloodGroupCountResponseEntity( p.bloodGroup, Count(p)) from Patient p group by p.bloodGroup")
